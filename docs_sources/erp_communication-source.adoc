...
IMPORTANT: Ab 2026-10-01 unterstützt der E-Rezept-Fachdienst die Version 3 der Payload der Communication Ressource. Da es zurzeit kein Pflicht gibt, können auch PayLoads der Version 1 weiterhin übermittelt werden. Die Version 3 der Payload enthält weitere Felder, um die Belieferung von E-Rezepten besser zu unterstützen. Es wird empfohlen, die Version 3 der Payload zu nutzen, aber beide Versionen müssen zuerst unterstützen werden, damit die Kompatibilität mit den verschiedenen Systemen gewährleistet ist. Es ist vorgesehen, dass die Version 1 der Payload zu einem späteren Zeitpunkt nicht mehr unterstützt wird, aber es gibt noch kein festes Datum dafür. +
Der Unterscheid ist hier zu finden: +
...
| supplyOptionsType
| ✅ freier String
| ✅ string (enum: delivery, shipment)
| ⚠️ Explizite Einschränkung auf erlaubte Werte

// --- Adresse / Lieferdaten ---
| name
| ✅ string
| ✅ string (maxLength≈100)
| ⚠️ Längenbeschränkung ergänzt
...
| country
| ✅ string
| ✅ string (Pattern: ISO-Ländercode, z.B. `[A-Z]{2}`)
| ⚠️ ISO-konforme Validierung

| hint
| ✅ string
| ✅ string (maxLength≈500)
| ⚠️ Einheitliche Begrenzung

| text
| ✅ string
| ✅ string (maxLength≈1000)
| ⚠️ jetzt zentral definiert und begrenzt

// --- Kontakt ---
| phone
| ✅ string
| ✅ string (Pattern: Telefonnummer, z.B. `^[0-9+\-()/ ]+$`)
| ⚠️ Formatvalidierung hinzugefügt

| mail
| ✅ string
| ✅ string (Pattern: RFC-konforme E-Mail)
| ⚠️ Strenge E-Mail-Validierung

// --- Identifikatoren ---
| transactionId
| ✅ string
| ✅ string (UUID / maxLength≈64)
| ⚠️ Format vereinheitlicht

| taskId
| ✅ string
| ✅ string (Pattern: ID-Format, maxLength≈64)
| ⚠️ Validierung ergänzt
...
| Reservation-Nachrichtentyp
| ❌
| ✅ eigenes Objekt (strukturierte Felder)
| ➕ Neue explizite Variante

| Trennung Delivery/Shipment
| ❌ implizit
| ✅ strikt getrennt (`oneOf`)
| ➕ Klare semantische Modellierung
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/05_request_RezeptZuweisen.json[]
----
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/06_response_RezeptZuweisen.json[]
----
...
[%collapsible]
====
NOTE: Die Validierung der JSON-payload wird nach folgendem Schema durchgeführt link:https://github.com/gematik/interactive-api-erp/blob/main/erp-fd-communications/CommunicationDispReqPayload.json[DispReq JSON-Schema].

NOTE: Weitere Beispiele der JSON-payload sind link:https://gematik.github.io/interactive-api-erp/erp-fd-communications/index.html[hier] zu finden.

NOTE: In diesem Fall hat das communicationType Feld im JSON-payload der Wert "order".
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/05_request_RezeptZuweisen.json[]
----
NOTE: Mit der Übergabe der Referenz auf den E-Rezept-Task inkl. des `AccessCodes` in `"reference": "Task/160.000.000.000.000.01/$accept?ac=*" ` ist die Apotheke berechtigt, das E-Rezept herunterzuladen und zu beliefern.
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/06_response_RezeptZuweisen.json[]
----
WARNING: Die derzeitige Spezifikation sieht vor, dass der E-Rezept Token in `.basedOn.reference` angegeben wird. Dieser Token entspricht nicht der FHIR-Spezifikation, wodurch die FHIR-Validatoren einen Fehler werfen.
...
[source,xml]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/03_request_PostPharmacyToPatient.xml[]
----
...
[source,xml]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/04_response_PostPharmacyToPatient.xml[]
----
...
[%collapsible]
====
Weitere Optionen für Nachrichten-Arten (mit Beispiele) von der Apotheke an den Versicherten sind link:https://gematik.github.io/interactive-api-erp/erp-fd-communications/index.html[hier] zu finden.

NOTE: Die Validierung der JSON-payload wird nach folgendem Schema durchgeführt link:https://github.com/gematik/interactive-api-erp/blob/main/erp-fd-communications/CommunicationReplyPayload.json[Reply JSON-Schema].
...
[source,xml]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/03_request_PostPharmacyToPatient.xml[]
----
NOTE: Die von der Apotheke übermittelte Antwort ist strukturiert in .payload.contentString nach gemSpec_eRp_DM abgelegt.
...
[source,xml]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/04_response_PostPharmacyToPatient.xml[]
----
NOTE: Der Server übernimmt beim Absenden der Nachricht den Sendezeitpunkt in die Communication-Ressource ` <sent value="2020-03-12T18:01:10+00:00" />`
...
==  Anwendungsfall Als Versicherter auf einer Nachricht einer Apotheke antworten
IMPORTANT: Diese Funktionalität ist nur für v3 verfügbar.

Als Versicherter möchte ich auf einer Nachricht einer Apotheke antworten, damit diese mein E-Rezept beliefern kann. Über einen JSON-Payload kann ich weitere Informationen oder Fragen zur Belieferung angeben.
...
NOTE: Interaktive Beispiele der JSON-payload sind link:https://gematik.github.io/interactive-api-erp/erp-fd-communications/index.html[hier] zu finden.

NOTE: In diesem Fall hat das communicationType Feld im JSON-payload der Wert "text".
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/09_request_MessagePharmacy.json[]
----
NOTE: Mit der Übergabe der Referenz auf den E-Rezept-Task inkl. des `AccessCodes` in `"reference": "Task/160.000.000.000.000.01/$accept?ac=*" ` ist die Apotheke berechtigt, das E-Rezept herunterzuladen und zu beliefern.
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/2026-10-01/api-examples/generated-examples/erp_communication/10_response_MessagePharmacy.json[]
----
WARNING: Die derzeitige Spezifikation sieht vor, dass der E-Rezept Token in `.basedOn.reference` angegeben wird. Dieser Token entspricht nicht der FHIR-Spezifikation, wodurch die FHIR-Validatoren einen Fehler werfen.
...
[source,json]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/07_response_GetMessages.json[]
----
...
[source,xml]
----
include::https://raw.githubusercontent.com/gematik/eRezept-Examples/{branch}/api-examples/generated-examples/erp_communication/08_response_GetAllMessages.xml[]
