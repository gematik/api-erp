...
Der E-Rezept-Fachdienst überprüft in 5-Minuten-Intervallen, ob mehrere Anfragen Prüfungsnachweise (pnw) mit dem Fehlercode "3" übermitteln (PN3). Sollte ein festgelegter Schwellenwert überschritten werden, akzeptiert der E-Rezept-Fachdienst auch Anfragen mit einem Prüfungsnachweis, der den Fehlercode 3 enthält.

Um diese Funktion zu nutzen, muss der neue URL-Parameter `kvnr` in der Anfrage enthalten sein. Dieser Parameter kann und kann unabhängig davon angegeben werden, ob ein Fehlercode 3 im pnw vorhanden ist oder nicht. Da das AVS nicht verpflichtet ist den Prüfungsnachweis zu überprüfen, kann der Parameter `kvnr` auch ohne Fehlercode 3 im pnw übermittelt werden.
