...
Gültige Werte für den Flowytype sind u.a. "160" für "Muster 16 (Apothekenpflichtige Arzneimittel)" und "200" für "PKV (Apothekenpflichtige Arzneimittel)". Das Rezept für private Versicherte wird mit dem Flowtype "200" ("PKV (Apothekenpflichtige Arzneimittel)") gestartet. Alle zulässigen Flowtype-Werte können dem Flowtype-CodeSystem (https://simplifier.net/erezept-workflow/gem-erp-cs-flowtype) entnommen werden. Der angegebene Flowtype wird in die Task Ressource unter Task.extension.flowType übernommen und bestimmt den Rezept-Typ. +
