...
==== Diagnose 
Eine einfach DNS record abfrage kann mögliche Probleme bei der Namensauflösung aufzeigen. 
Öffnen Sie hierfür ein Terminal unter Windows und geben folgende Zeile ein: `nslookup erp.zentral.erp.splitdns.ti-dienste.de`
...
Im Falle einer OPNsense Firewall ist dies im Web Interface in wenigen Schritten getan. Öffnen Sie die Weboberfläche und klicken Sie auf der linken Seite auf [Dienste], weiter auf [Unbound DNS] dort auf [Erweitert].
Suchen Sie hier den Punkt [private Domains], tragen Sie dort die Adresse `splitdns.ti-dienste.de` ein und bestätigen Sie die Eingabe mit Enter. Führen Sie dies genauso mit der `*.telematik` Adresse durch. Wenn Sie
dies abgeschlossen haben, scrollen Sie an das Ende der Seite und klicken Sie [Anwenden]. Der Dienst übernimmt nun die Einstellungen. 
