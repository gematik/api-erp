...
==== Problemstellung
Der VSDM++ Abruf für das Feature "eGK in der Apotheke" läuft in einigen Fällen in einen Timeout. Das stellt sich dadurch da, dass der Aufruf ca. 25 Sekunden dauert und mit einem Prüfungsnachweis mit Code "3" quittiert wird.

Dies betrifft AVS-Systeme mit einem secunet-Konnektor.
