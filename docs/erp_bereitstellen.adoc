...
WARNING: Es ist nicht vorgesehen oder gewollt, dass der Leistungserbringer mehrere Tasks erstellt, um mehrere Rezept-IDs zu generieren und diese zwischenzeitlich lokal zu speichern, um sie später mit einem E-Rezept-Bundle zu befüllen und zu aktivieren. Vielmehr soll der Task erst dann erstellt werden, wenn das E-Rezept-Bundle vollständig vorliegt und die QES erstellt werden kann. Das Erstellen eines Tasks ohne zeitnahe Aktivierung führt zu „verwaisten“ Tasks auf dem E-Rezept-Fachdienst, die nicht mehr verwendet werden können. Dies führt zu einem unnötigen Verbrauch von Rezept-IDs, die nicht unbegrenzt verfügbar sind. Aktuell ist vorgesehen, dass Rezept-IDs frühestens nach 10 Jahren wiederverwendet werden sollen.
