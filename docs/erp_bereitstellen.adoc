...
----
<1> Dieser Parameter steuert den Typ des dem Task zugrunde liegenden Workflows. In Stufe 1 des E-Rezepts wird das Muster16 umgesetzt. Daraus ergibt sich, dass der Versicherte diese Rezepte nur in einer Apotheke einlösen kann.
...
TIP: Der Konnektor wählt standardmäßig ein passendes kryptografisches Verfahren, es kann jedoch mit dem Parameter `crypt` in SignDocument auch gemäß der Spezifikation in gemSpec_Kon#TAB_KON_862-01 konkret gewählt werden (z.B. ECC, falls das Verhalten der verschiedenen Algorithmen ausprobiert werden soll).
  ...
  <SOAP-ENV:Body>
    <m:SignDocument xmlns:m="http://ws.gematik.de/conn/SignatureService/v7.4">
      ...
      <m:SignRequest RequestID="Doc1">
        <m:OptionalInputs>
          <m2:SignatureType>urn:ietf:rfc:5652</m2:SignatureType> <1>
          <m:IncludeEContent>true</m:IncludeEContent> <2>
        </m:OptionalInputs>
        <m:Document ID="CMS-Doc1" ShortText="a CMSDocument2sign"> <3>
...
ICA8L2VudHJ5Pg0KPC9CdW5kbGU+
        ...
        <m:IncludeRevocationInfo>true</m:IncludeRevocationInfo> <4>
      </m:SignRequest>
    </m:SignDocument>
  </SOAP-ENV:Body>
</SOAP-ENV:Envelope>
...
<4> Das Flag `IncludeRevocationInfo` weist den Konnektor an, die OCSP-Statusprüfung des Signaturzertifikats in den Signaturcontainer mit einzubetten. Dadurch kann die spätere Signaturprüfung ohne erneute Statusabfrage erfolgen.
...
[%autowidth]
|===
|URI        |https://erp.zentral.erp.splitdns.ti-dienste.de/Task/4711/$activate
|Method     |POST
|HTTP Header |
----
Content-Type: application/fhir+xml; charset=UTF-8
...
Content-Type: application/fhir+xml;charset=utf-8 
Content-Location: 
  https://erp.zentral.erp.splitdns.ti-dienste.de/Task/4711/_history/2 
<Task xmlns="http://hl7.org/fhir">
  <id value="4711"/>
  <meta>
    <versionId value="2"/>
...
[%autowidth]
|===
|URI        |https://erp.zentral.erp.splitdns.ti-dienste.de/Task/4711/$abort
|Method     |POST
|HTTP Header |
----
Authorization: Bearer eyJraWQ.ewogImL2pA10Qql22ddtutrvx4FsDlz.rHQjEmB1lLmpqn9J <1>
