...
Vor dem Zugriff auf den E-Rezept-Fachdienst muss ein Prüfungsnachweis mittels VSD-Online-Prüfung durch den Konnektor erstellt werden. Dafür wird die ReadVSD-Operation des Konnektors mittels folgendem Aufruf gestartet.
...
NOTE: Alle gelieferten Tasks haben den Status `<status value="ready" />` und können im Folgenden durch die Apotheke einzeln abgerufen und beliefert werden, siehe link:erp_abrufen.adoc#e-rezept-abrufen[E-Rezept abrufen^].
