...
`PrescriptionID` und `AccessCode` wird in der Task-Ressource nun mit einem aktualisierten NamingSystem ausgewiesen.
...
WARNING: Ab 01.07.2023 müssen PVS Systeme in der Lage sein PrescriptionID und AccessCode mit den neuen NamingSystems auszulesen. Falls im Verordnungsdatensatz ein KBV_Bundle der Version 1.0.2 erstellt wird, müssen die alten NamingSystems für `PrescriptionID` und `AccessCode` eingetragen werden.
