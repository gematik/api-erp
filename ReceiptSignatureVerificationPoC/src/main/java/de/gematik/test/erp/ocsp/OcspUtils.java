...
public class OcspUtils {
    ...
    @SneakyThrows
    private static OCSPReq generateOCSPRequest(X509Certificate certificate, @NonNull BigInteger serialNumber)  {
        val nonce = BigInteger.valueOf(System.currentTimeMillis());
        val ext = new Extension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce, true,
                new DEROctetString(nonce.toByteArray()));

        val ocspReqBuilder = new OCSPReqBuilder()
                .addRequest(new JcaCertificateID(
    ...
    public static OcspRespCache generateOcspRespCache(
            @NonNull OCSPResp ocspResp,
            @NonNull BigInteger signerSerialNumber){
